                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-6">
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                    required autofocus autocomplete="name" value="{{ $product->name }}" />
                            </div>
                            <div class="col-6">
                                <x-input-label for="brand" :value="__('Brand')" />
                                <x-text-input id="brand" name="brand" type="text" class="mt-1 block w-full"
                                    required autofocus autocomplete="description" value="{{ $product->brand }}" />
                            </div>
                            ...
                            <div class="grid grid-cols-1">
                                ...
                                <x-text-area rows="4" cols="50" id="description" name="description"
                                    ...
                                    autocomplete="description" value="{{ $product->description }}" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <x-text-input id="price" name="price" type="number" class="mt-1 block w-full"
                                    required autofocus autocomplete="description" value="{{ $product->price }}" />
                            </div>

                            <div>
                                <x-input-label for="stock" :value="__('Stock')" />
                                <x-text-input id="stock" name="stock" type="number" class="mt-1 block w-full"
                                    required autofocus autocomplete="description" value="{{ $product->stock }}" />
                            </div>

                            <div>
                                <x-input-label for="discount" :value="__('discount')" />
                                <x-text-input id="discount" name="discount" type="number" class="mt-1 block w-full"
                                    required autofocus autocomplete="discount" value="{{ $product->discount }}" />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="categories" name="categories"
                                    class="mt-2 block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    {{ $categoriaProducto = optional($product->find($product->id)->categories->first())->id }}
                                    @foreach ($category as $name => $id)
                                        <option {{ $id == $categoriaProducto ? 'selected' : '' }}
